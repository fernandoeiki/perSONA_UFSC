...
public:
	 ...
	 */
	inline virtual double Length( ) const { return sqrt( x * x + y * y + z * z ); };
	 ...
	 */
	inline double Dot( const VAVec3& vDotProduct ) const { return ( x * vDotProduct.x + y * vDotProduct.y + z * vDotProduct.z ); };
...
inline VABASE_API void operator+=( VAVec3& oVec, const VAVec3& v3Rhs )
{
	oVec.x += v3Rhs.x;
	oVec.y += v3Rhs.y;
	oVec.z += v3Rhs.z;
...
inline VABASE_API VAVec3 operator-( const VAVec3& oSummand1, const VAVec3& oSummand2 )
{
	VAVec3 vDiff = oSummand1;
	vDiff -= oSummand2;
	return vDiff;
};

//!< Scalar multiplication for vectors
inline VABASE_API void operator*=( VAVec3& oVec, const double& dRhs )
{
	oVec.x *= dRhs;
	oVec.y *= dRhs;
	oVec.z *= dRhs;
...
public:
	...
	inline double Length( ) const { return std::sqrt( x * x + y * y + z * z + w * w ); };
...
struct VABASE_API CVAAudioRendererInfo
{
	...
	inline CVAAudioRendererInfo( ) : bEnabled( false ), bOutputDetectorEnabled( false ), bOutputRecordingEnabled( false ) { };
};
...
public:
	...
	inline CVADirectivityInfo( ) : iID( -1 ), iClass( UNSPECIFIED ), iReferences( 0 ) { };
...
public:
	...
	//! Default constructor
	inline CVASignalSourceInfo( ) : iReferences( 0 ), iType( UNSPECIFIED ) { };
	 ...
	 */
	inline CVASignalSourceInfo( const std::string& sID, const int iType, const std::string& sName, const std::string& sDesc, const std::string& sState,
	                            const int iReferences )
	    : sID( sID )
	    , iType( iType )
	    , sName( sName )
	    , sDesc( sDesc )
...
public:
	...
	inline CVASceneInfo( ) : bEnabled( true ) { };
...
public:
	...
	inline CVASoundReceiverInfo( ) : iID( -1 ), iDirectivityID( -1 ), iAuraMode( -1 ), dSensivitity( 1.0f ), bEnabled( true ) { };
...
public:
	...
	inline CVASoundPortalInfo( ) : iID( -1 ), iNextPortalID( -1 ), iSoundSourceID( -1 ), iSoundReceiverID( -1 ), iMaterialID( -1 ) { };
...
public:
	...
	inline CVAAcousticMaterial( ) : iID( -1 ) { };
...
public:
	...
	inline CVAGeometryMesh( ) : iID( -1 ), bEnabled( true ) { };
};
