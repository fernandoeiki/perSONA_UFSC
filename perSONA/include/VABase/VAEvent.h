...
public:
	...
	static const uint64_t SOUND_RECEIVER_CHANGED_MUTING = VA_BIT64( 44 );

	// Next free slot: 45

	//!< Selector mask for signal source related events
	static const uint64_t SIGNALSOURCE_EVENT = SIGNALSOURCE_CREATED | SIGNALSOURCE_DELETED | SIGNALSOURCE_REGISTERED | SIGNALSOURCE_UNREGISTERED |
	                                           SIGNALSOURCE_STATE_CHANGED | SOUND_SOURCE_CHANGED_SIGNALSOURCE;

	//!< Selector mask for sound source related events
	static const uint64_t SOUNDSOURCE_EVENT = SOUND_SOURCE_CREATED | SOUND_SOURCE_DELETED | SOUND_SOURCE_CHANGED_NAME | SOUND_SOURCE_CHANGED_SIGNALSOURCE |
	                                          SOUND_SOURCE_CHANGED_AURALIZATIONMODE | SOUND_SOURCE_CHANGED_SOUND_POWER | SOUND_SOURCE_CHANGED_MUTING |
	                                          SOUND_SOURCE_CHANGED_POSE | SOUND_SOURCE_CHANGED_DIRECTIVITY;

	//!< Selector mask for sound receiver related events
	static const uint64_t SOUND_RECEIVER_EVENT = SOUND_RECEIVER_CREATED | SOUND_RECEIVER_DELETED | SOUND_RECEIVER_CHANGED_NAME | SOUND_RECEIVER_CHANGED_AURALIZATIONMODE |
	                                             SOUND_RECEIVER_CHANGED_DIRECTIVITY | SOUND_RECEIVER_CHANGED_POSE | SOUND_RECEIVER_CHANGED_MUTING;

	//!< Selector mask for portal related events
	static const uint64_t PORTAL_EVENT = SOUND_PORTAL_CHANGED_NAME | SOUND_PORTAL_CHANGED_PARAMETER;


	//!< Selector mask for sound source modification event
	static const uint64_t SOUNDSOURCE_MODIFICATION = SOUND_SOURCE_CHANGED_NAME | SOUND_SOURCE_CHANGED_SIGNALSOURCE | SOUND_SOURCE_CHANGED_AURALIZATIONMODE |
	                                                 SOUND_SOURCE_CHANGED_SOUND_POWER | SOUND_SOURCE_CHANGED_MUTING | SOUND_SOURCE_CHANGED_POSE |
	                                                 SOUND_SOURCE_CHANGED_DIRECTIVITY;

	//!< Selector mask for sound receiver modification event
	static const uint64_t SOUND_RECEIVER_MODIFICATION = SOUND_RECEIVER_CHANGED_NAME | SOUND_RECEIVER_CHANGED_AURALIZATIONMODE | SOUND_RECEIVER_CHANGED_DIRECTIVITY |
	                                                    SOUND_RECEIVER_CHANGED_MUTING | SOUND_RECEIVER_CHANGED_POSE;
