...
bool VistaMatrix<Type>::SingularValueDecompositionTest( const VistaMatrix<Type> & matU, const VistaMatrix<Type> & matW, const VistaMatrix<Type> & matV, Type epsilon )
{
	...
	VistaMatrix<Type> matTransposedV( matV );
	matTransposedV.Transpose();
	VistaMatrix<Type> matTransposedU( matU );
	matTransposedU.Transpose();

	// test for U * W * V^T == A
	VistaMatrix<Type> matTest1 = (matU * matDiagW * matTransposedV) - (*this);
	...
	VistaMatrix<Type> matEn( n, n );
	for( iRow = 0; iRow < n; ++iRow )
		matEn[iRow][iRow] = 1.0;

	// test for V^T * V == E
	VistaMatrix<Type> matTest2 = ( matTransposedV * matV ) - matEn;
	...
	// test for U^T * U = E'
	VistaMatrix<Type> matTest3 = matTransposedU * matU - matEn;
