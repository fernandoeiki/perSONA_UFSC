...
public:
	 ...
	 *
	 */
	virtual int Initialize( const std::string& sServerAddress, const int iServerPort = VANET_DEFAULT_PORT, const int iHeadChannelMode = VA_HC_USE_EXISTING,
	                        const int iExceptionHandlingMode = EXC_CLIENT_THROW, const bool bBufferSynchronizedCommands = true ) = 0;
	...
	public:
		 ...
		 */
		inline CEvent( const int iInitType, const std::string& sInitServer ) : iType( iInitType ), sServer( sInitServer ) { };
	};
